...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  if (!text) throw `*[❗ملاحضة❗] يرجى اضافة عنوان الفيديو مع الرمز*\n\n*—◉ مثال:*\n*${usedPrefix + command} Stormy - Amira* `;
  try {
    const yt_play = await search(args.join(' '));
    let additionalText = '';
    if (command === 'ytadoc' || command == 'pdoc') {
      additionalText = 'audio 🔊';
    } else if (command === 'ytvdoc' || command == 'pdoc2') {
      additionalText = 'video 🎥';
    }
    const texto1 = `*◉——⌈🔊 جاري تحميل الملف 🔊⌋——◉*\n
❏ 📌 *العنوان:* ${yt_play[0].title}
❏ 📆 *تاريخ التنزيل:* ${yt_play[0].ago}
❏ ⌚ *المدة:* ${secondString(yt_play[0].duration.seconds)}
❏ 👀 *المشاهدات:* ${`${MilesNumber(yt_play[0].views)}`}
❏ 👤 *المالك:* ${yt_play[0].author.name}`.trim();
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            await conn.reply(m.chat, '*[❗] خطأ، لا يمكن تحميل الملف*', m);
          }
        }
      }
    }
    if (command == 'ytvdoc' || command == 'pdoc2') {
      try {
        const qu = '360';
        const q = qu + 'p';
        const v = yt_play[0].url;
    ...
    throw '*[❗ملاحضة❗] خطأ، يرجى المحاولة لاحقا*';
  }
};
handler.help = ['ytadoc', 'ytvdoc'].map((v) => v + ' < busqueda >');
handler.tags = ['downloader'];
handler.command = /^(pdoc|pdoc2|ytadoc|ytvdoc)$/i;

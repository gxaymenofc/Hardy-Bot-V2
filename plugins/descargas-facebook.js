...
const handler = async (m, {conn, args, command, usedPrefix}) => {
  ...
  try {
    await m.reply(`[⏳] Charging...*`);
    const d2ata = await facebook.v1(args[0]);
    let r2es = '';
    if (d2ata.urls && d2ata.urls.length > 0) {
      r2es = `${d2ata.urls[0]?.hd || d2ata.urls[1]?.sd || ''}`;
    }
    conn.sendFile(m.chat, r2es, 'error.mp4', `_*https://instagram.com/gxaymenofc*_`, m);
  } catch (err1) {
    try {
          ...
          try {
            ...
            try {
              ...
              try {
                ...
                for (const {url, isVideo} of result.reverse()) await conn.sendFile(m.chat, url, `facebook.${!isVideo ? 'bin' : 'mp4'}`, '*[⏳] Charging..._', m);
              } catch (err6) {
